...
> [!NOTE]
> `independentModules.json`, `independentModules.jsonc` and `independentModules.yaml` are also supported. See an example in the 🎮 [Playground](https://github.com/Igorkowalski94/eslint-plugin-project-structure-playground#root) for eslint-plugin-project-structure rules.
  ...
  modules: [
...

    {
      ...
      allowImportsFrom: [
        ...
        // But we won't be able to import Feature2 private files and folders.
...
By writing `{yourKey}` you refer to a particular key in the `reusableImportPatterns` object, you can use this reference in `reusableImportPatterns` and in [`allowImportsFrom`](#allow-imports-from).
...
By default, at least **two** common parts are required, [`baseUrl`](#path-aliases) is not taken into account.
This will make your rule work **recursively**/apply to all nestings.
...
If your `tsconfig` is located elsewhere or if `baseUrl` and `paths` are in a different file than `tsconfig.json`, you can specify its location here.
...
Debug mode showing the current file path, import path, [`allowImportsFrom`](#allow-imports-from), [`{family}`](#family), and [`{dirname}`](#dirname) for a given import.
