...
const PlayerCard = React.memo(
  ({ playerName, matchId, highlight }: PlayerCardProps) => {
    ...
    return (
      <Paper bg={highlight ? "myColor.9" : "dark.8"} radius="md">
        <Group>
                ...
                .map((stats) => {
                  return (
                    <PlayerCard
                      ...
                      matchId={matchId}
                      highlight={playerFilter.includes(stats.playerName)}
                    />
                  );
                })}
...
const MatchDateGroup = React.memo(
  ({
    dateString,
    dateRange,
    teamFilter,
    ...
    const filteredMatches = matchData.filter((match) => {
      ...
      const passesDateFilter =
        dateRange[0] && dateRange[1]
          ? matchDate >= dateRange[0] && matchDate <= dateRange[1]
          : true;
      const passesTeamFilter =
        teamFilter.length === 0
          ? true
          : teamFilter.includes(match.team1Name) ||
            teamFilter.includes(match.team2Name);
      const passesPlayerFilter =
        playerFilter.length === 0
          ? true
          : match.team1Players
              .concat(match.team2Players)
              .some((player) => playerFilter.includes(player));
      const passesMapFilter =
        mapFilter.length === 0 ? true : mapFilter.includes(match.map);
      return (
        passesDateFilter &&
        passesTeamFilter &&
        passesPlayerFilter &&
    ...
    const filteredScrims = scrims.filter((scrim) => {
      return scrim.matchIds.some((matchId) =>
        filteredMatches.some((match) => match.matchId === matchId)
      );
    });
...
export const MatchesPage = () => {
  ...
  const mapFilter = useMemo(() => searchParams.get("maps")?.split(",") || [], [
    searchParams,
  ]);
  ...
  return (
    <div>
      ...
      <Group grow>
        <MultiSelect
          placeholder="Search by team name, player name, etc."
          searchable
          data={[
        ...
        <DatePickerInput
          type="range"
          clearable
          placeholder="Show matches between dates"
          value={dateRange}
