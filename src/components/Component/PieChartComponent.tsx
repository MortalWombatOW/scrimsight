  ...
  const renderActiveShape = (props) => {
    ...
    return (
      <g>
        ...
        <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill="#333">{`${formatFn(payload.payload)}`}</text>
        {/* <text
        x={ex + (cos >= 0 ? 1 : -1) * 12}
        y={ey}
        dy={18}
          ...
          <PieChart
            ...
            <Pie
              ...
              {data.map((entry, index) => (
                <Cell key={`cell-${index}`} className={groupColorClass(entry[colorKey] as string)} />
              ))}
            </Pie>
