...
import {
  summarizeTransform, FieldDescription, DatasetTransform, filterTransform,
  applyTransforms, fieldTransforms, sortTransform,
} from '../../lib/data/data';
...
const RangeSelector = (props: RangeSelectorProps) => {
  const {
    data, addTransform, close, closeInner, field,
  } = props;
  ...
  const xScale = (x: number) => xPadding + ((x - min) / (max - min)) * (width - 2 * xPadding);
  const xScaleInverse = (x: number) => ((x - xPadding) / (width - 2 * xPadding))
  * (max - min) + min;
    ...
    <div>
      <svg
        ...
        onMouseMove={(e) => {
          if (startActive) {
            const { movementX } = e;
            const newStart = Math.max(min, Math.min(end, xScaleInverse(xScale(start) + movementX)));
            setStart(newStart);
          }
          if (endActive) {
            const { movementX } = e;
            const newEnd = Math.max(start, Math.min(max, xScaleInverse(xScale(end) + movementX)));
            setEnd(newEnd);
          }
        }}
        onMouseUp={() => {
      ...
      >
        ...
        {[...Array(numTicks).keys()].map((i) => {
          const tickX = (i * (width - 2 * xPadding)) / (numTicks - 1) + xPadding;
          return (
            <g key={i}>
              <line
                x1={tickX}
        ...
        <line x1={xScale(min)} y1={25} x2={xScale(max)} y2={25} className="RangeSelector-axis" />
        <line x1={xScale(start)} y1={25} x2={xScale(end)} y2={25} className="RangeSelector-selected" />
        <text
          x={xScale(start)}
          y={15}
          textAnchor="middle"
      ...
      <div className="RangeSelector-buttons">
        <Button
          onClick={() => {
            closeInner();
            close();
            addTransform(filterTransform(
              (row: DataRow) => Number.parseFloat(row.get(field)!) >= start
...
const FilterPopover = (props: FieldPopoverChildProps) => {
  ...
  if (field.type === 'number') {
    const data: number[] = dataset.getField(field.name)!
  ...
  return (
    <div>
      ...
      <div className="groupByButtonGroup">
        {dataset.getFields().filter((f) => f.name !== field.name).map((f) => (
          <Button
            key={f.name}
            onClick={() => {
            ...
            variant={groupByFields.includes(f.name) ? 'contained' : 'outlined'}
          >
            {f.name}
          </Button>
        ))}
...
const SortPopover = (props: FieldPopoverChildProps) => {
  const {
    field, addTransform, close, closeInner,
  } = props;
