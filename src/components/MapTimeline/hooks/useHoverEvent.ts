...
export const useHoverEvent = () => {
  ...
  const handleMouseMove = useCallback(
    (e: FederatedPointerEvent, events: PlayerEvent[], interactionEvents: PlayerInteractionEvent[], ultimateEvents: UltimateEvent[], timeToX: (t: number) => number, xToTime: (x: number) => number) => {
