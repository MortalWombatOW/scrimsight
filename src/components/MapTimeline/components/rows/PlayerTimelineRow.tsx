  ...
  ({width, height, y, playerName, events = [], interactionEvents = [], ultimateEvents = [], dimensions, useWindowScale = true, label, labelWidth, onLabelWidthChange, onDelete}) => {
    ...
    return (
      <BaseTimelineRow width={width} height={height} y={y} dimensions={dimensions} useWindowScale={useWindowScale} label={label} labelWidth={labelWidth} onLabelWidthChange={onLabelWidthChange} onDelete={onDelete}>
        <Graphics
          interactive={true}
          hitArea={new PIXI.Rectangle(0, 0, timelineWidth, height)}
          mousemove={(e) => {
        ...
        />
        {hoverEvent && <Text text={hoverEvent.text} style={tooltipStyle} x={hoverEvent.x} y={hoverEvent.y - 15} anchor={new PIXI.Point(0.5, 1)} />}
      </BaseTimelineRow>
    );
  },
);
