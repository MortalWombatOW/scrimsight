...
export const RoundTimelineRow = memo<TimelineRowProps>(({
  width,
  height,
  y,
  timelineData,
    ...
    >
      <Graphics
        draw={g => {
          ...
          timelineData.roundTimes.forEach((round, index) => {
            ...
            g.drawRect(
              timeToX(round.roundStartTime),
              0,
              timeToX(round.roundSetupCompleteTime) - timeToX(round.roundStartTime),
              height
            );
            // Active section
            g.beginFill(0x4CAF50, 0.15);  // Reduced opacity
            g.drawRect(
              timeToX(round.roundSetupCompleteTime),
              0,
              timeToX(round.roundEndTime) - timeToX(round.roundSetupCompleteTime),
              height
            );
          });
        }}
      ...
      {timelineData.roundTimes.map((round, index) => (
        <React.Fragment key={`round-${index}`}>
          <Text
            text="Setup"
            style={phaseStyle}
            x={(timeToX(round.roundStartTime) + timeToX(round.roundSetupCompleteTime)) / 2}
            y={2}
          ...
          <Text
            text="Active"
            style={phaseStyle}
            x={(timeToX(round.roundSetupCompleteTime) + timeToX(round.roundEndTime)) / 2}
            y={2}
            anchor={new PIXI.Point(0.5, 0)}
          />
        </React.Fragment>
      ...
      {timelineData.roundTimes.map((round, index) => (
        <Text
          ...
          text={`Round ${index + 1}`}
          style={textStyle}
          x={timeToX(round.roundStartTime)}
          y={height + 2}
          anchor={new PIXI.Point(0.5, 0)}
        />
      ))}
