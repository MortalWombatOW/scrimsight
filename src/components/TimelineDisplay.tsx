...
import { formatTime, getHeroImage } from "@library";
import { groupedEvents, KillLogEvent, OffensiveAssistLogEvent } from "@atoms";
...
export const TimelineDisplay = (): ReactNode => {
  ...
  const killAssistGroups = useAtomValue(groupedEvents.atom);
  ...
  const visibleKillAssistGroups = useMemo(() => {
    if (!loadedData?.matchData) return [];
    return killAssistGroups.filter(
      (g) =>
        g.matchId === loadedData.matchData.matchId &&
        g.matchTime >= currentTimeRange.start &&
        g.matchTime <= currentTimeRange.end
    );
  }, [killAssistGroups, loadedData?.matchData, currentTimeRange]);
  ...
  const renderKillAssistGroup = (
    ...
    const EventCluster = ({
      ...
      events: Array<{ type: "kill" | "assist"; data: KillLogEvent | OffensiveAssistLogEvent }>;
      isTeam1: boolean;
    }) => {
      ...
      return (
        ...
        >
          ...
          >
            {events.map((event, i) => {
              if (event.type === "kill") {
                const kill = event.data as KillLogEvent;
                return (
                  <EventBadge
                ...
                const assist = event.data as OffensiveAssistLogEvent;
                return (
                  <EventBadge
