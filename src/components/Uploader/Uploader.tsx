...
const Uploader: React.FC = () => {
  ...
  const rowCounts = fileNames.reduce(
    (acc, fileName) => {
      const out = objectStoreNames.reduce(
        (acc2, storeName) => {
  ...
  return (
    <>
      ...
      <UploadProgressModal isOpen={modalOpen} setIsOpen={setModalOpen} initialRowCounts={initialRowCounts} rowCounts={rowCounts} objectStoreNames={objectStoreNames} />
    </>
  );
};
