...
import { getHeroImage, type MatchData } from "@library";
...
export const TimelineEvents = (): ReactNode => {
  ...
  const { matchData, events }: { matchData: MatchData; events: TimelineEvent[] } = loadedData;
      ...
      <div className="max-h-[400px] overflow-y-auto bg-base-100">
        {events.map((event, index) => {
            ...
            if (p.playerInteractionEventType.toLowerCase().includes("killed")) {
              ...
              const targetEvent = events.find(
                (e) =>
                  ...
                  e.type === "playerEvent" &&
                  e.playerEvent?.eventType.toLowerCase().includes("death")
              );
          ...
          // Player Event (Death, Swap)
            ...
            return (
              <TimelineItem
                key={`player-${p.matchId}-${p.matchTime}-${p.playerName}-${p.eventType}-${index}`}
                // time={p.playerEventTime} // Time removed
                playerName={p.playerName}
                playerHero={p.playerHero}
                eventType={p.eventType}
                isTeam1={isTeam1}
                isSelected={selectedEventId === `${p.matchId}-${p.matchTime}-${p.playerName}-${p.eventType}`}
                // No targetName/targetHero for these events
                onClick={() => setSelectedEventId(`${p.matchId}-${p.matchTime}-${p.playerName}-${p.eventType}`)}
                onMouseLeave={() => setSelectedEventId(null)}
              />
            );
          }
