...
//   const xMin = Math.min(
//     ...(dataset.map((row) => Number.parseFloat(row.get(xField))) as number[]),
//   );
//   const xMax = Math.max(
//     ...(dataset.map((row) => Number.parseFloat(row.get(xField))) as number[]),
//   );
//   const yMin = Math.min(
//     ...(dataset.map((row) => Number.parseFloat(row.get(yField))) as number[]),
//   );
//   const yMax = Math.max(
//     ...(dataset.map((row) => Number.parseFloat(row.get(yField))) as number[]),
//   );
...
//         const s = sizeScale(
//           hasSizeField ? Number.parseFloat(point.get(sField!)) : 0,
//         );
//         return (
//           <circle
...
//   const {dataset, cField, colorScale, xOffset, yOffset} = props;

//   const hasColorField = cField !== null;

//   const uniqueCategories = [
//     ...new Set(dataset.map((row: DataRow) => row.get(cField!) as string)),
//   ];
...
//       dataset.getFields().filter((field) => field.name === cField)[0].type ===
//       'number';
...
//     const sMin = Math.min(
//       ...(dataset.map((row) =>
//         Number.parseFloat(row.get(sField!)),
//       ) as number[]),
//     );
//     const sMax = Math.max(
//       ...(dataset.map((row) =>
...
//           label="X field"
//           value={xField}
//           onChange={(e) => setXField(e.target.value as string)}>
//           {dataset
//             .getFields()
...
//           label="Y field"
//           value={yField}
//           onChange={(e) => setYField(e.target.value as string)}>
//           {dataset
//             .getFields()
//             .filter((field) => field.type === 'number')
//             .map((field) => (
