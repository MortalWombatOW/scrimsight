...
import {DataRowBySpecName} from './types';

export default class ResultCache {
  private data: DataRowBySpecName = {};
  ...
  /**
   * Check if a query has results stored in the cache.
   * @param name - The name of the query.
   * @returns True if results exist, false otherwise.
   */
  hasResults(name: string) {
    return Array.isArray(this.data[name]);
  }
   ...
   * @returns True if the query is running, false otherwise.
   */
  isRunning(queryName: string) {
    return (
      this.data[queryName] !== undefined &&
      Object.keys(this.data[queryName]).length === 0
    );
  }

  /**
   * Check if a query has been fulfilled or not.
   * @param key - The name of the query.
   * @returns True if the query is not done, false otherwise.
   */
  notDone(key: string) {
    return this.data[key] === undefined || this.isRunning(key);
  }

  // ... other caching-related methods as needed
}
