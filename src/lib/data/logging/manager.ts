...
import {DataAndSpecName} from './spec';
...
export async function parallelProcessLogLines(
  lines: string[],
  mapId: number,
  numWorkers: number = 4,
): Promise<DataAndSpecName[]> {
  // Create workers
  const workers: Worker[] = [];
  for (let i = 0; i < numWorkers; ++i) {
    // workers.push(new LogWorker());
    workers.push(new Worker(new URL('./log.worker.ts', import.meta.url)));
  }
  ...
  const transfer = (rank: number, [lines]: any) => {
    ...
    const transferData =
      rank === root ? [stringsArrayToArrayBuffer(lines)] : [];
  ...
  try {
    ...
    const parsedLogData: DataAndSpecName[] = result
      .flat()
      .reduce((acc, val) => {
        // merge the data when the specName is the same
        const {specName, data} = val;
        const existingData = acc.find(
          (mergedDataAndSpec: DataAndSpecName) =>
            mergedDataAndSpec.specName === specName,
        );
        if (existingData) {
